...
function Ovo({ match }) {
  const ovoData = Object.values(data).filter(
    (s) => s.food_menu.includes("오보") === true
  );
  ...
  const perPageData = ovoData.slice(indexOfFirstData, indexOfLastData);
  return (
    <section className="food">
      <div className="food__content">
        <div className="food__map">
          <ReactMapGL
            ...
            minZoom={7}
          >
            <Pins data={ovoData} onClick={setPopupInfo} />
            {popupInfo && (
              <Popup
                tipSize={5}
                anchor="top"
      ...
      <Page pageData={[page, setPage]} data={ovoData} />
    </section>
  );
}
