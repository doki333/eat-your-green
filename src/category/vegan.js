...
import Pins from "../category/marker";
...
function Vegan({ match }) {
  const veganData = Object.values(data).filter(
    (s) => s.food_menu.includes("비건") === true
  );
  ...
  const perPageData = veganData.slice(indexOfFirstData, indexOfLastData);
  return (
    <section className="food">
      <div className="food__content">
        <div className="food__map">
          <ReactMapGL
            ...
            minZoom={7}
          >
            <Pins data={veganData} onClick={setPopupInfo} />
            {popupInfo && (
              <Popup
                tipSize={5}
                anchor="top"
      ...
      <Page pageData={[page, setPage]} data={veganData} />
    </section>
  );
}
