...
function Lacto({ match }) {
  const lactoData = Object.values(data).filter(
    (s) => s.food_menu.includes("락토") === true
  );
  ...
  const perPageData = lactoData.slice(indexOfFirstData, indexOfLastData);
  return (
    <section className="food">
      <div className="food__content">
        <div className="food__map">
          <ReactMapGL
            ...
            minZoom={7}
          >
            <Pins data={lactoData} onClick={setPopupInfo} />
            {popupInfo && (
              <Popup
                tipSize={5}
                anchor="top"
      ...
      <Page pageData={[page, setPage]} data={lactoData} />
    </section>
  );
}
