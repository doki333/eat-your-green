...
function Pesce({ match }) {
  const pesceData = Object.values(data).filter(
    (s) => s.food_menu.includes("페스코") === true
  );
  ...
  const perPageData = pesceData.slice(indexOfFirstData, indexOfLastData);
  return (
    <section className="food">
      <div className="food__content">
        <div className="food__map">
          <ReactMapGL
            ...
            minZoom={7}
          >
            <Pins data={pesceData} onClick={setPopupInfo} />
            {popupInfo && (
              <Popup
                tipSize={5}
                anchor="top"
      ...
      <Page pageData={[page, setPage]} data={pesceData} />
    </section>
  );
}
